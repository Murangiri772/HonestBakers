...
@file:JvmName("UserBakesScreenKt")
...
@Composable
fun UserBakesScreen(navController: NavController, viewModel: BakesViewModel) {
    val userBakes by viewModel.allBakes.observeAsState(emptyList())

    var searchQuery by remember { mutableStateOf("") }

    val filteredBakes = userBakes.filter {
    ...
    Scaffold(
        topBar = {
            Column {
                TopAppBar(
                    title = { Text("Welcome To Our Menu", fontSize = 20.sp) },
        ...
        bottomBar = {
            UserBottomNavigationBar1(navController)
        ...
        Column(
            ...
            LazyColumn {
                items(filteredBakes.size) { index ->
                    UserBakesItem(navController, filteredBakes[index], viewModel)
                }
            }
        }
    }
...
@RequiresApi(Build.VERSION_CODES.Q)
@Composable
fun UserBakesItem(navController: NavController, bakes: Bakes, viewModel: BakesViewModel) {
...
@Composable
fun UserBottomNavigationBar1(navController: NavController) {
